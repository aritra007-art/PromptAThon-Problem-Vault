...
export const DemoWalkthrough: React.FC<DemoWalkthroughProps> = ({
  ...
  return (
    <div className="space-y-6">
      {/* Hero Banner */}
      <div className="p-6 rounded-2xl bg-gradient-to-br from-slate-900 via-slate-900 to-cyan-950/40 border border-slate-800 shadow-xl relative overflow-hidden">
        <div className="max-w-3xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-950/80 border border-cyan-800/80 text-cyan-300 text-xs font-semibold mb-3">
          ...
          <div className="flex flex-wrap items-center gap-3 mt-5">
            <button
              ...
              disabled={isBusy}
              className="px-4 py-2 rounded-xl text-xs font-semibold bg-cyan-500 hover:bg-cyan-400 text-slate-950 shadow-lg shadow-cyan-500/20 transition-all flex items-center gap-2 cursor-pointer font-mono disabled:opacity-50"
            >
            ...
            <button
              ...
              className="px-3 py-2 rounded-xl text-xs font-medium text-slate-400 hover:text-slate-200 hover:bg-slate-800 transition-colors flex items-center gap-1.5"
            >
          ...
          {steps.map(s => {
            ...
            return (
              <button
                ...
                className={`py-1.5 px-2 rounded-lg text-center text-[10px] font-mono transition-all border ${
                  isCurrent
        ...
        <div className="lg:col-span-2 p-6 rounded-xl bg-slate-900 border border-slate-800 space-y-4">
          ...
          <div className="flex items-center gap-3 pt-2">
            <button
              onClick={() => executeStep(currentStep)}
              disabled={isBusy}
              className="px-4 py-2 rounded-lg text-xs font-bold bg-cyan-600 hover:bg-cyan-500 text-white shadow-md transition-all flex items-center gap-2 cursor-pointer disabled:opacity-50"
            >
            ...
            {currentStep < steps.length && (
              <button
                ...
                className="px-3 py-2 rounded-lg text-xs font-medium text-slate-400 hover:text-white bg-slate-800 hover:bg-slate-700 transition-colors"
              >
                Skip to Next →
              </button>
            )}
          ...
          {demoObject ? (
            <div className="space-y-3 font-mono text-xs">
              ...
              <div className="space-y-1.5">
                <span className="text-[10px] text-slate-400 uppercase block">Replicas by Node</span>
                {demoObject.replicas.map(r => {
                  ...
                  return (
                    <div
                      key={r.nodeId}
                      className="p-2 rounded bg-slate-950/80 border border-slate-800 flex items-center justify-between text-[11px]"
                    >
